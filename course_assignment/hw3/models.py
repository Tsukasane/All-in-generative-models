...
def up_conv(in_channels, out_channels, kernel_size, stride=1, padding=1, # to replace transpose conv
...
class DCGenerator(nn.Module):
    ...
    def __init__(self, noise_size, conv_dim=64, norm='instance'):
        ...
        self.up_conv1 = nn.Conv2d(100, 256, 3, 2, 4, bias=None)
        self.up_conv2 = up_conv(256, 128, 3, 1, 1, 2, norm, 'relu')
        self.up_conv3 = up_conv(128, 64, 3, 1, 1, 2, norm, 'relu')
        self.up_conv4 = up_conv(64, 32, 3, 1, 1, 2, norm, 'relu')
        self.up_conv5 = up_conv(32, 3, 3, 1, 1, 2, norm=None, activ='tanh')
...
class ResnetBlock(nn.Module):
    ...
    def __init__(self, conv_dim, norm, activ):
        super().__init__()
        self.conv_layer = conv( # save size of input and output
...
class CycleGenerator(nn.Module):
    ...
    def __init__(self, conv_dim=64, init_zero_weights=False, norm='instance'):
        ...
        self.up_conv1 = up_conv(64, 32, 3, 1, 1, 2, norm, 'relu')
...
class DCDiscriminator(nn.Module):
    ...
    def __init__(self, conv_dim=64, norm='instance'):
        ...
        self.conv5 = conv(256, 1, 4, 1, 0, None) # no norm, no activ

    def forward(self, x):
        """Forward pass
            x is (B, C, H, W).
            Output BS x 1 x 1 x 1
        """
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.conv3(x)
